# seven-day-cumulative-usage Specification

## Purpose

定义 `sevenDayCumulativeUsagePct` 派生指标：把 Claude 7 天额度的锯齿波（`sevenDayUsagePct` 百分比快照）还原成区间内的累计真实使用量。该指标在 `ccus aggregate` 读时从 `rawEvents` 计算，输出到 `daily.csv` / `weekly.csv` 与 `ccus aggregate serve` 看板，不持久化、不进入 `ccus export` 的 bundle 字段集合。
