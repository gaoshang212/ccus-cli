...
- 收录 GPT-6 Sol、GPT-6 Luna 和 Claude Opus 5.5 的标准 API 价格，自 2026-09-22 起用于成本统计和价格页面。
- 补齐 Sol、Luna 的长上下文价格及 Opus 5.5 的缓存读写价格，兼容模型别名与推理后缀。

### 变更

- 价格目录版本更新为 `2026-09-23`。

## [0.2.19] - 2026-09-20

### 变更

- 同步、导出和个人看板统计前自动修复跨天滞后的 session 修改时间，减少消息统计遗漏。

## [0.2.18] - 2026-09-20

### 新增

- 新增 `ccus sessions repair`，支持按 Codex/Claude 修复会话文件修改时间、预览、自定义时间差阈值、硬链接去重及耗时显示，默认仅修复跨天差异。

## [0.2.17] - 2026-09-14

### 新增

- JSON 成本汇总增加未定价模型名称及请求数。

### 变更

- 导出版本升至 11，聚合继续兼容 v6–v10。

## [0.2.16] - 2026-09-14

### 新增

- 新增 `gpt-5.3-codex-spark` 的 API 等效成本统计，按 `gpt-5.3-codex` 单价估算：每百万 token 输入 $1.75、输出 $14、缓存读取 $0.175。文档与价格页注明为估算价格，非 Spark 官方公开 API 定价。

## [0.2.15] - 2026-09-07

### 新增

- 新增 GPT-6 Astra 的 API 等效成本统计，兼容模型别名与推理强度后缀，支持长上下文计价并同步展示模型价格。

## [0.2.14] - 2026-08-28

### 变更
- 优化 `ccus sync` 与个人 dashboard 的统计性能：按时间范围跳过无关历史文件，复用周/日组合扫描，并对 Codex 与 Orca 的硬链接、相同内容及完整前缀副本走快速去重；真实数据下同步导出与看板响应均降至约 2 秒，输出契约不变。
- `ccus sync` 成功输出新增总耗时，统一以秒显示并保留三位小数；后台 `__sync` 继续保持静默。

## [0.2.13] - 2026-08-25

### 修复
- Codex app-server RPC 返回错误、空额度或直接抛错时统一回退 `wham/usage` HTTP；两路均失败时跳过本次额度事件，不再使用过期缓存额度。
...
### 新增
- `ccus sessions` 新增 Codex 原始会话打包，并保持 Claude 归档兼容。
